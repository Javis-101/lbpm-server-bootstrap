...
class SourcePublicationContractTests(unittest.TestCase):
    def test_release_representation_has_validated_version(self) -> None:
        self.assertEqual(
            (ROOT / "VERSION").read_text(encoding="utf-8").strip(), "1.0.3"
        )

    def test_release_representation_excludes_future_path_guard(self) -> None:
        self.assertFalse((ROOT / "installer/lib/path_safety.sh").exists())
